...
- **GA4 tracking:** every `compact` and `links` button (via a `track` field), the `#save-contact` button, and the share button carry an inline `onclick` firing `gtag('event','click_boton_tarjeta',{nombre_boton})`, guarded by `if(typeof gtag !== 'undefined')`. `nombre_boton` values: `telefono`, `mail`, `guardar_contacto`, `whatsapp`, `linkedin_personal`, `linkedin_agencia`, `ver_soluciones`, `agendar_reunion`, `compartir`. `gtag` is defined by the GA4 `gtag.js` tag in `Layout.astro` (see Deployment). The *Ver Soluciones Comerciales* `?ref=nfc` param is what attributes web visits coming from the card — GA4 captures it in `page_location`.
...
- **Analytics = GA4 `gtag.js` only** (`G-BTDMD5T6S8`), as two `is:inline` scripts high in `Layout.astro`'s `<head>`, so it covers BOTH pages. This defines the global `gtag` the `/tarjeta` click events rely on. GTM was tried earlier and removed — do NOT re-add a GTM container or a second Google tag (avoids double-counting). Tracking only sends data from the live site, not localhost.

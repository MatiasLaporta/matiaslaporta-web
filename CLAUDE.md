...
- **GA4 tracking:** every `links` button (via a `track` field) and the share button carry an inline `onclick` firing `gtag('event','click_boton_tarjeta',{nombre_boton})`, guarded by `if(typeof gtag !== 'undefined')`. `gtag` is defined by the GA4 `gtag.js` tag in `Layout.astro` (see Deployment). The *Ver Soluciones Comerciales* `?ref=nfc` param is what attributes web visits coming from the card — GA4 captures it in `page_location`.
